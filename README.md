...
.
├── notebooks/
│   ├── 01_data_prep.ipynb          # splits, stratification, seed
│   ├── 02_classical_baseline.ipynb # TF-IDF + logistic regression
│   ├── 03_roberta_finetune.ipynb   # transformer fine-tuning
│   └── 04_llm_prompting.ipynb      # zero-shot and few-shot
├── src/                            # shared metrics, threshold search, IO
├── results/                        # metric tables, threshold sweeps
├── figures/
├── requirements.txt
└── README.md
```
...
Download the Jigsaw dataset from Kaggle and place `train.csv` in `data/`. Run the notebooks in order. Seed 42 is fixed for splitting, sampling, and training.

The LLM notebook requires an OpenAI API key:

```bash
export OPENAI_API_KEY="your-key"
```

Fine-tuning RoBERTa needs a GPU. It ran in roughly 150 minutes for 3 epochs at batch size 8 in this setup.
